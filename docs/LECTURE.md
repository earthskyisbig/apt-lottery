...
- **없는 값을 지어내지 않기.** 원천에 없으면 "정보 없음". 경쟁률·가점은 접수 종료 후에만 채워짐 → 미집계는 `status`로 구분하고 빈 표 대신 안내 문구.
...
| 경쟁률·가점 표가 비어 있다 | `fetch_stats.py` 미실행, 또는 접수 종료 전이라 값이 없음(`"-"`/null) | 통계 스크립트 실행 → `01_collector_stats.json` 생성. 신규 공고는 마감 후 재실행해야 채워짐 |
