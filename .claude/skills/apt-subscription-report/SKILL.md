...
- 지시: `subscription-collect` 스킬의 번들 스크립트 2개를 순서대로 — ①`scripts/fetch_subscriptions.py --out _workspace --lookback-days 90`(5개 유형 분양공고) ②`scripts/fetch_stats.py --workspace _workspace --skip-not-started`(공고별 주택형·경쟁률·당첨가점·특공신청현황, 경쟁률 서비스 36148). `_workspace/01_collector_*.json` + `01_collector_stats.json` 저장.
- `run_in_background: false` (다음 단계가 결과에 의존). 통계 수집은 공고 300건 기준 3~5분 걸린다.
- **인증 체크**: 반환된 meta의 `auth_ok`가 false면(키 미설정/401) 파이프라인을 계속 진행하되, Phase 4에서 사용자에게 `.env`의 `ODCLOUD_SERVICE_KEY` 설정을 안내한다. 통계만 실패하면 공고 리포트는 만들고 "통계 미수집"을 notes로 노출.

## Phase 2: 분석·필터
`Agent` 도구로 `subscription-analyst` 호출 (`model: "opus"`).
- 지시: `subscription-analyze` 스킬의 `scripts/analyze.py --workspace _workspace`를 실행해 수도권 필터·D-day·규제/국민민영·주택형·경쟁률·가점·특공 해석 → `_workspace/02_analyst_summary.json`. 출력 요약·notes를 검토해 이상(0건·파싱 실패·통계 부재)을 보고.
- 사용자가 필터/마감기준 커스텀을 줬으면 지시에 전달(스크립트 상수 수정 후 재실행).
...
- 지시: `subscription-report-html` 스킬의 `scripts/build_report.py`(실행일 기준 D-day 재계산)로 `02_analyst_summary.json`(+있으면 `03_match_personal.json`) → `reports/청약리포트_{날짜}.html` 생성. `03`이 있으면 최상단에 "🎯 내 조건 맞춤" 섹션. 생성 후 브라우저 렌더(섹션 5개·표 행 수·콘솔 에러 0) 확인.
- 반환: 저장 경로 + 핵심 요약(신규 N·임박 M·맞춤 TOP).

## Phase 3.5: 자격진단 웹앱 빌드 (Phase 3과 병렬 가능)
`subscription-webapp` 스킬의 `scripts/build_webapp.py --workspace _workspace --out reports`를 실행해 `reports/청약자격진단.html`을 갱신한다. 데이터 주입만 하는 결정적 스크립트라 에이전트 없이 오케스트레이터가 직접 실행해도 된다. 실행 후 브라우저(Playwright)로 한 번 열어 콘솔 에러 0·공고 건수 표시를 확인한다.

## Phase 4: 사용자 보고
- 리포트·웹앱 경로를 알려주고, 핵심 요약(진행·예정 공고 수, 마감 임박 건, 지난 접수 결과 중 주목할 미달/고경쟁 단지)을 3줄 이내로 브리핑.
...
## 테스트 시나리오
- **정상 흐름**: 트리거 → 수집(공고 N건 + 통계 호출 M회) → 분석(수도권 진행 K건·임박 J건·결과집계 R건) → HTML 리포트 + 웹앱 생성 → 경로+요약 보고.
- **통계만 실패**: `fetch_stats.py` 401/네트워크 → `01_collector_stats.json` 없음 → analyst notes "통계 없음" → 리포트 경쟁률 섹션에 안내 박스, 나머지는 정상.
